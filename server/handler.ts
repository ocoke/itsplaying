...
export default async function handler(req: Request): Response | Promise<Response> {
  ...
  if (path.startsWith('/api/get.png')) {
    const webstackScreenshotAPI =
      Deno.env.get('WEBSTACK_SCREENSHOT_API') || 'https://webstack-screenshot.vercel.app'

    let rqUrl = webstackScreenshotAPI + '/'
    rqUrl +=
      '?url=' +
      encodeURIComponent(Deno.env.get('ITSPLAYING_FRONTEND') || 'https://itsplaying.deno.dev') +
      '/share?id=' +
      url.searchParams.get('id')
      + '&viewport=' + url.searchParams.get('view')
